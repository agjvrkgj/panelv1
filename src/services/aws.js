...
async function swapNodeIp(node, awsInstanceId, awsType, awsRegion, awsAccountId) {
  ...
  try {
    ...
    // 2. 更新数据库（同时同步同机 IPv6 节点的 ssh_host）
    db.updateNode(node.id, { host: result.newIp, ssh_host: result.newIp });
    const allNodes = db.getAllNodes();
    const peerNodes = allNodes.filter(n => n.id !== node.id && (n.ssh_host || n.host) === (node.ssh_host || node.host) && n.protocol !== node.protocol);
    for (const peer of peerNodes) {
      db.updateNode(peer.id, { ssh_host: result.newIp });
    }
    l('数据库已更新' + (peerNodes.length > 0 ? ` (含 ${peerNodes.length} 个同机节点)` : ''));
