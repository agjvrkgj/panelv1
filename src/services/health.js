...
// 每节点用户滑动窗口：nodeId → { userLastSeen: Map<userId, timestamp>, nodeName, ts }
const _nodeOnlineMap = new Map();
const USER_ONLINE_TTL = 120000; // 用户 2 分钟内有流量即算在线
const NODE_ONLINE_TTL = 180000; // 节点 3 分钟无上报则过期
...
function updateOnlineCache(nodeId, trafficRecords, routeCtx = null) {
  ...
  // 兼容旧格式：若本轮无协议信息，则对双协议伙伴继续镜像在线人数
  ...
  // 滑动窗口：合并本轮活跃用户到节点的 userLastSeen，而非替换
  const trackedNodes = [node, ...peerNodes];
  for (const n of trackedNodes) {
    let entry = _nodeOnlineMap.get(n.id);
    if (!entry) {
      entry = { userLastSeen: new Map(), nodeName: n.name, ts: now };
      _nodeOnlineMap.set(n.id, entry);
    }
    entry.ts = now;
    entry.nodeName = n.name;
    // 将本轮活跃用户的 lastSeen 更新为当前时间
    const activeUsers = nodeUserIdsMap.get(n.id) || new Set();
    for (const uid of activeUsers) {
      entry.userLastSeen.set(uid, now);
    }
    // 清除过期用户（超过 USER_ONLINE_TTL 无流量）
    for (const [uid, ts] of entry.userLastSeen) {
      if (now - ts > USER_ONLINE_TTL) entry.userLastSeen.delete(uid);
    }
  }

  // 清理过期节点（超过 NODE_ONLINE_TTL 无上报）
  for (const [nid, entry] of _nodeOnlineMap) {
    if (now - entry.ts > NODE_ONLINE_TTL) _nodeOnlineMap.delete(nid);
  }

  // 从所有活跃节点重新计算全局在线数据
  rebuildOnlineCache(now);
}

// 从 _nodeOnlineMap 重建全局在线缓存
function rebuildOnlineCache(now) {
  const allUsers = new Map(); // userId → username
  const nodes = [];
  for (const [nid, entry] of _nodeOnlineMap) {
    nodes.push({ nodeId: nid, nodeName: entry.nodeName, count: entry.userLastSeen.size });
    for (const uid of entry.userLastSeen.keys()) {
      if (!allUsers.has(uid)) {
        const u = db.getUserById(uid);
        if (u) allUsers.set(uid, u.username);
      }
    }
  }
  const users = [];
  for (const [id, username] of allUsers) {
    users.push({ id, username });
  }
  _onlineCache.full = { total: users.length, users, nodes };
  _onlineCache.summary = { online: users.length, nodes: nodes.length };
  _onlineCache.ts = now;
}
...
function updateFromAgentReport(nodeId, reportData) {
  ...
  // 更新在线用户缓存（即使无流量也需要更新，以便将该节点在线人数清零）
  updateOnlineCache(nodeId, trafficRecords, routeCtx);
