...
const { getClientIp, isCloudflareIp } = require('../utils/clientIp');
...
function invalidateSubCache(token) {
  if (!token) { _subCache.clear(); return; }
  // 按 token 前缀删除所有 clientType 变体的缓存
  const prefix = token + ':';
  const v6prefix = 'v6:' + token + ':';
  for (const key of _subCache.keys()) {
    if (key.startsWith(prefix) || key.startsWith(v6prefix)) {
      _subCache.delete(key);
    }
  }
}
...
router.get('/sub/:token', subLimiter, (req, res) => {
  ...
  // 拦截 Cloudflare IP（防止订阅转换/聚合服务拉取）
  if (process.env.SUB_BLOCK_CF_IP !== 'false' && isCloudflareIp(clientIP)) {
    logSubAccessEventSafe({
      token, route: 'sub', ip: clientIP, ua, clientType: '',
      userId: null, result: 'deny', reason: 'cloudflare_ip', httpStatus: 403,
    });
    return res.status(403).type('text').send('订阅请求被拒绝');
  }
...
router.get('/sub6/:token', subLimiter, (req, res) => {
  ...
  if (process.env.SUB_BLOCK_CF_IP !== 'false' && isCloudflareIp(clientIP)) {
    logSubAccessEventSafe({
      token, route: 'sub6', ip: clientIP, ua, clientType: '',
      userId: null, result: 'deny', reason: 'cloudflare_ip', httpStatus: 403,
    });
    return res.status(403).type('text').send('订阅请求被拒绝');
  }
...
module.exports.invalidateSubCache = invalidateSubCache;
