...
function updateNodeBasicInfo(req, res) {
  ...
  if (node) {
    ...
    // IPv6 双协议节点的 ssh_host 指向 IPv4 同机节点，编辑时不应覆盖
    const updates = { name: trimmedName, host: trimmedHost };
    if (node.ip_version !== 6 || node.ssh_host === node.host) {
      updates.ssh_host = trimmedHost;
    }
