...
// Cloudflare 公开 IP 段 (https://www.cloudflare.com/ips/)
const CF_IPV4_CIDRS = [
...
// 预编译 CIDR 为 [min, max] 范围
const _cfRanges = CF_IPV4_CIDRS.map(cidr => {
  const [ip, bits] = cidr.split('/');
  const mask = (~0 << (32 - parseInt(bits, 10))) >>> 0;
  const base = ipToInt(ip) & mask;
  return [base, base | (~mask >>> 0)];
});

function isCloudflareIp(ip) {
  const normalized = normalizeIp(ip);
  if (!normalized || normalized.includes(':')) return false; // 仅检查 IPv4
  const num = ipToInt(normalized);
  return _cfRanges.some(([min, max]) => num >= min && num <= max);
}
