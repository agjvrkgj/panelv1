...
test('GET /sub/:token rejects unknown UA in enforce mode', async (t) => {
  ...
  const oldSignMode = process.env.SUB_LINK_SIGN_MODE;
  process.env.SUB_CLIENT_FILTER_MODE = 'enforce';
  process.env.SUB_LINK_SIGN_MODE = 'off';

  const panelRoutePath = require.resolve('../src/routes/panel');
  const dbPath = require.resolve('../src/services/database');
  const healthPath = require.resolve('../src/services/health');

  const prevPanel = require.cache[panelRoutePath];
  const prevDb = require.cache[dbPath];
  const prevHealth = require.cache[healthPath];

  // Mock database with minimal methods needed by panel.js sub route
  const mockDb = {
  ...
  const mockHealth = {
    getOnlineCache: () => ({ full: null, summary: null }),
  };

  delete require.cache[panelRoutePath];
  require.cache[dbPath] = { id: dbPath, filename: dbPath, loaded: true, exports: mockDb };
  require.cache[healthPath] = { id: healthPath, filename: healthPath, loaded: true, exports: mockHealth };
  ...
  t.after(() => {
    ...
    if (oldSignMode == null) delete process.env.SUB_LINK_SIGN_MODE;
    else process.env.SUB_LINK_SIGN_MODE = oldSignMode;
    delete require.cache[panelRoutePath];
    if (prevDb) require.cache[dbPath] = prevDb;
    else delete require.cache[dbPath];
    if (prevHealth) require.cache[healthPath] = prevHealth;
    else delete require.cache[healthPath];
  });
...
test('csrfProtection rejects json request when Origin is missing or mismatched', async (t) => {
  ...
  assert.deepEqual(await r1.json(), { error: 'CSRF 校验失败：Origin 不匹配且 Token 无效' });
  ...
  assert.deepEqual(await r2.json(), { error: 'CSRF 校验失败：Origin 不匹配且 Token 无效' });
});
